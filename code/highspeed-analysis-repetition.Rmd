...
subset(dt_pred_rep_timecourses, classification == "ovr") %>%
  ...
  transform(change = ifelse(change == 2, "forward", change)) %>%
  transform(change = ifelse(change == 9, "backward", change)) %>%
  dplyr::rename(event = "position") %>%
  dplyr::rename(interference = "change") %>%
  dplyr::rename(tr = "seq_tr") %>%
...
subset(dt_pred_rep_mean_prob_plot, classification == "ovr") %>%
  ...
  dplyr::rename(event = "position_label") %>%
...
subset(dt_pred_rep_a2_sub, classification == "ovr") %>%
  ...
  filter(change %in% c(2, 9)) %>%
  transform(change = as.numeric(as.character(change))) %>%
  transform(change = ifelse(change == 2, "forward", change)) %>%
  transform(change = ifelse(change == 9, "backward", change)) %>%
  dplyr::rename(interference = "change") %>%
  dplyr::rename(event = "position_label") %>%
...
subset(dt_pred_rep_step_trans, classification == "ovr" & change %in% c(2,9)) %>%
  select(-classification, -num_data, -mean_num_label) %>%
  filter(change %in% c(2, 9)) %>%
  transform(change = as.numeric(as.character(change))) %>%
  transform(change = ifelse(change == 2, "forward", change)) %>%
  transform(change = ifelse(change == 9, "backward", change)) %>%
  dplyr::rename(interference = "change") %>%
  write.csv(., file = file.path(path_sourcedata, "source_data_figure_s10e.csv"),
            row.names = FALSE)
```
...
subset(
  ...
  transform(change = ifelse(change == 9, "backward", change)) %>%
  dplyr::rename(interference = "change") %>%
  write.csv(., file = file.path(path_sourcedata, "source_data_figure_s10d.csv"),
            row.names = FALSE)
```
